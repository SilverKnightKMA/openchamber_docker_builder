...
FROM node:26-bookworm-slim@sha256:d2ec0a1766c01dad04a185c2d5558b0adace167a7f1758ce80f0017698431d06 AS node-runtime
...
FROM docker:29.4.3-dind@sha256:685b91dca8eab7de1dce1c303dbb7a763e4082d6a60db10968adf3295fbd2495 AS docker-dind
...
COPY --from=toolchain package.json package-lock.json /opt/openchamber/opencode-core/
COPY --from=toolchain package.json package-lock.json /opt/openchamber/managed-tools/
COPY --from=toolchain go.mod go.sum tools.go /opt/openchamber/managed-tools/
COPY --from=toolchain managed-tools/ /opt/openchamber/managed-tools/managed-tools/
COPY --from=toolchain scripts/managed-npm-tools.mjs scripts/managed-go-tools.mjs scripts/managed-mounted-tools.mjs scripts/managed-tools-output.mjs scripts/managed-tools-status.mjs /opt/openchamber/managed-tools/scripts/
...
RUN node - <<'JS'
const fs = require('fs');
const base = '/opt/openchamber/opencode-core';
const pkgPath = `${base}/package.json`;
const lockPath = `${base}/package-lock.json`;
const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
const lock = JSON.parse(fs.readFileSync(lockPath, 'utf8'));
const opencodeVersion = pkg.dependencies?.['opencode-ai'];
if (!opencodeVersion || lock.packages?.['']?.dependencies?.['opencode-ai'] !== opencodeVersion) {
  throw new Error('opencode-ai must be pinned in both package.json and package-lock.json');
}
pkg.description = 'Pinned opencode core package baked into the OpenChamber image.';
pkg.dependencies = { 'opencode-ai': opencodeVersion };
delete pkg.devDependencies;
delete pkg.optionalDependencies;
delete pkg.peerDependencies;
lock.packages[''].dependencies = { 'opencode-ai': opencodeVersion };
fs.writeFileSync(pkgPath, `${JSON.stringify(pkg, null, 2)}\n`);
fs.writeFileSync(lockPath, `${JSON.stringify(lock, null, 2)}\n`);
JS
RUN npm ci --omit=dev --prefix /opt/openchamber/opencode-core \
  && ln -sf /opt/openchamber/opencode-core/node_modules/.bin/opencode /usr/local/bin/opencode \
  && rm -rf /root/.npm
...
ENV MANAGED_NPM_PREFIX=/home/openchamber/.npm-global
ENV BUN_INSTALL=/home/openchamber/.bun
ENV CARGO_HOME=/home/openchamber/.cargo
ENV MANAGED_CARGO_HOME=/home/openchamber/.cargo
ENV GOPATH=/home/openchamber/.go
ENV GOBIN=/home/openchamber/.go/bin
ENV MANAGED_GO_ROOT=/home/openchamber/.local/go
ENV PYTHONUSERBASE=/home/openchamber/.local/pip
ENV MANAGED_RELEASE_BIN_DIR=/home/openchamber/.local/bin
ENV RUSTUP_HOME=/home/openchamber/.rustup
ENV MANAGED_RUSTUP_HOME=/home/openchamber/.rustup
...
ENV PATH=/home/openchamber/.local/bin:/home/openchamber/.npm-global/bin:/home/openchamber/.local/go/bin:/home/openchamber/.go/bin:/home/openchamber/.cargo/bin:/home/openchamber/.local/pip/bin:/home/openchamber/.bun/bin:/opt/openchamber/npm-global/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
